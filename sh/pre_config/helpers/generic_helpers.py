...
def get_batch_execution_id(augmented_site_level_config):
    ...
    if batch_execution_id is None:
        raise Exception('Could not find HTCondor-Batch in the lightweight components section of the '
                        'site level configuration file. It is required to configure the HTCondor-Worker containers.'
                        'Please add a HTCondor-Batch component as described in '
                        'https://github.com/WLCG-Lightweight-Sites/simple_htcondor_batch'
                        'Then rollback and re-run the framework. '
                        'Please note that the SIMPLE Grid Framework\'s HTCondor-Worker components can only work with '
                        'SIMPLE Grid Framework\'s HTCondor Batch system at present.')
